...
@Component({
    ...
    <section class="panel">
      ...
      <form *ngIf="showCreatePanel" class="form-grid" [formGroup]="createForm" (ngSubmit)="create()" novalidate>
        ...
        <div class="field field--price-mode">
          ...
          <div class="toggle-cluster">
            ...
            <label class="toggle-check" *ngIf="canViewCostPrice">
              <input type="radio" name="createPriceMode" [checked]="createPriceMode === 'margin'" (change)="setCreatePriceMode('margin')" />
              <span class="toggle-check__track"></span>
        ...
        <div class="price-preview-card">
          ...
          <strong>{{ publicPricePreview(createForm) | currency:'USD':'symbol':'1.2-2' }}</strong>
          ...
          <small *ngIf="!canViewCostPrice">Precio visible para venta y catalogo.</small>
        </div>
        ...
        <table class="product-table">
          ...
          <tbody>
            <tr *ngFor="let product of products; trackBy: trackByProduct">
              ...
              <td class="meta">&#36;{{ visiblePrice(product) | number: '1.2-2' }}</td>
              <td class="meta" *ngIf="canViewCostPrice">&#36;{{ (product.costPrice ?? 0) | number: '1.2-2' }}</td>
              <td class="meta">{{ product.unitPrice == null ? '-' : ('$' + (product.unitPrice | number: '1.2-2')) }}</td>
        ...
        <form *ngIf="modalMode !== 'stock'" [formGroup]="editForm" (ngSubmit)="submitModal()" novalidate>
          ...
          <div class="field field--price-mode">
            ...
            <div class="toggle-cluster">
              ...
              <label class="toggle-check" *ngIf="canViewCostPrice">
                <input type="radio" name="editPriceMode" [checked]="editPriceMode === 'margin'" (change)="setEditPriceMode('margin')" [disabled]="modalMode === 'delete'" />
                <span class="toggle-check__track"></span>
                <span class="toggle-check__text">Margen de ganancia</span>
              </label>
            </div>
          ...
          <div class="price-preview-card price-preview-card--modal">
            ...
            <strong>{{ publicPricePreview(editForm) | currency:'USD':'symbol':'1.2-2' }}</strong>
            ...
            <small *ngIf="!canViewCostPrice">Compatibilidad legacy activa via alias price.</small>
          </div>
          <div class="modal__actions">
  ...
  styles: [`
    ...
    .form-grid{position:relative;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:1rem;align-items:start}.field{position:relative;display:block}.field--wide{grid-column:span 4}.field--price-primary,.field--price-mode{grid-column:span 2}
    ...
    .table-wrap{overflow-x:auto;position:relative}.product-table{width:100%;border-collapse:collapse;min-width:1180px}.product-table th{color:var(--text-subtle);text-align:left;padding:0 .8rem .8rem 0;border-bottom:1px solid var(--border)}
    ...
    @media (max-width:860px){.form-grid{grid-template-columns:1fr}.field--wide,.field--price-primary{grid-column:auto}.stock-form,.stock-summary{grid-template-columns:1fr}.pagination{display:grid;align-items:stretch}.pagination__actions{align-items:stretch}}
  `]
})
export class ProductsComponent implements OnInit {
  ...
  private toProductRequest(form: FormGroup) {
    ...
    const publicPrice = Number(raw.publicPrice ?? raw.price ?? 0);
